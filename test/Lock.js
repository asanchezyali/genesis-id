...
describe("GenesisId", function () {
  let genesisId;
  let owner, user1, user2;

  beforeEach(async function () {
    [owner, user1, user2] = await hre.ethers.getSigners();
    const IdentityContract = await hre.ethers.getContractFactory("GenesisId");
    genesisId = await IdentityContract.deploy();
    await genesisId.deployed();
  });
  ...
  it("Should create a new user", async function () {
    ...
    const user = await genesisId.getUserByUsername("johndoe");
  ...
  it("Should fetch user data by address", async function () {
    ...
    const user = await genesisId.connect(user1).getUserByAddress(user1.address);
    expect(user.basicInfo.firstName).to.equal("John");
    expect(user.basicInfo.lastName).to.equal("Doe");
    expect(user.basicInfo.email).to.equal("john@example.com");
  });
  ...
  it("Should set and get visibility settings", async function () {
    ...
    await genesisId.setVisibility("johndoe", false, false, false, false, false);

    const updatedVisibility = await genesisId.getVisibility("johndoe");
